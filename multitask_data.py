...
class MultitaskDataset():
    ...
    def load_hp(self, conf):
        ...
        train = self.process_hp(dataset["train"], tokenizers[conf.tokenizer])
        val = self.process_hp(dataset["validation"], tokenizers[conf.tokenizer])
    ...
    def load_ag(self, conf):
        ...
        train = self.process_ag(dataset["train"], tokenizers[conf.tokenizer])
        val = None
    ...
    def load_bcc(self, conf):
        ...
        train = self.process_bbc([[ex[1][:512], l2i[ex[2]]] for ex in train_data], tokenizers[conf.tokenizer])
        val = None
        ...
        test = self.process_bbc([[ex[1][:512], l2i[label[1]]] for ex, label in zip(test_ex, test_labels) if ex[0] == label[0]], tokenizers[conf.tokenizer])
